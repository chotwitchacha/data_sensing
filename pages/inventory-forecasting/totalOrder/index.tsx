...
import { TableDataForecasting } from '@components/Component'
import { Typography, TextField, IconButton, Tooltip, Menu, Box, Grid, MenuItem, Button, ListItemText, OutlinedInput } from '@material-ui/core'
...
import { Checkbox, Collapse, Table, TableBody, TableCell, TableHead, TableRow } from '@mui/material'
...
const columns = [
  {
    ...
    history: [
      {
        orderDate:'Dec 01, 2021',
        saleOrder: 'Dec 01, 2021',
        onThewWay: '15',
        recieveOreder: '5',
        replenishStockDate:'Dec 15, 2021'
      },
      {
        orderDate:'Nov 16, 2021',
        saleOrder: '24',
        onThewWay: '14',
        recieveOreder: '10',
        replenishStockDate:'Dec  30, 2021'
      }
    ]
  },
    ...
    options: {
      ...
      customBodyRender: (value) => {
        return (
          <Typography component={'span'} noWrap={false}>
            <span>Brand: </span>
            <span style={{ fontWeight: 'bold' }}>{value.brand}</span>
            <span>, Product Name: </span>
            <span style={{ fontWeight: 'bold' }}>{value.product_name}</span>
            <span>, Color: </span>
...
const reports = ({ tableData, trendData, storeData, groupData, classChartData, brandData, filterApiRes }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  const updateDataCallback = async (page: number, rowsPerPage: number, filterList: any) => {
    ...
    axios.post(`http://localhost:8000/get_invent_report_page`, body)
      .then(function (res) {
        // handle success
        const data = res.data
  ...
  const HeaderElements = () => {
      ...
      switch (caseFilter) {
        case 'onHand':
          if (event.target.value === "")
            ...
            setFilter({ ...filter, onHand: { ...filter.onHand, operator: event.target.value } })
          break;
        case 'consumptionRate':
          if (event.target.value === "")
            setFilter({ ...filter, consumptionRate: { ...filter.consumptionRate, operator: event.target.value, value: '' } })
          else
            setFilter({ ...filter, consumptionRate: { ...filter.consumptionRate, operator: event.target.value } })
          break;
        case 'expectedReplenishmentStock':
          if (event.target.value === "")
            setFilter({ ...filter, expectedReplenishmentStock: { ...filter.expectedReplenishmentStock, operator: event.target.value, value: '' } })
          else
            setFilter({ ...filter, expectedReplenishmentStock: { ...filter.expectedReplenishmentStock, operator: event.target.value } })
          break;
        case 'expectedReorderDate':
          if (event.target.value === "")
            setFilter({ ...filter, expectedReorderDate: { ...filter.expectedReorderDate, operator: event.target.value, value: null } })
          else
            setFilter({ ...filter, expectedReorderDate: { ...filter.expectedReorderDate, operator: event.target.value } })
          break;
      }
    }
    ...
    const handleValueChange = (event: React.ChangeEvent<HTMLInputElement>, dateValue: Date | null, caseFilter) => {
      ...
      switch (caseFilter) {
        ...
        case 'expectedReplenishmentStock':
          if (event.target.value === '' || re.test(event.target.value) && event.target.value[0] !== '0') {
            setFilter({ ...filter, expectedReplenishmentStock: { ...filter.expectedReplenishmentStock, value: event.target.value } })
          }
          break;
        case 'expectedReorderDate':
      ...
      >
        <Box className={classes.filterBox}>
          ...
          >
            ...
            <Box width="20%">
              <Button color="primary" onClick={resetFilter}>Reset</Button>
            </Box>
  ...
  const onRowsClick = (rowData: string[], rowMeta: { dataIndex: number, rowIndex: number }) => {
    ...
    router.push({
      pathname: `/inventory-forecasting/history/${description.brand}/${description.product_name}/${description.sku}`
    })
  }
  ...
  const getMuiTheme = () => (createTheme as any)({
    overrides: {
      MUIDataTable: {
        responsiveScroll: {
              ...
              <Collapse in={open} timeout="auto" unmountOnExit>
                <Box sx={{ margin: 1 }}>
                  ...
                  <Table size='small' aria-label="purchases">
                    ...
                    <TableBody>
                      {columns.map((item) => (
                        <>{item.history.map((index)=> (
                          <TableRow key={index.orderDate}>
                            <TableCell component='th' scope='row'>
                              {index.orderDate}
                            </TableCell>
                            <TableCell> {index.saleOrder}</TableCell>
                            <TableCell> {index.onThewWay}</TableCell>
                            <TableCell> {index.recieveOreder}</TableCell>
                            <TableCell> {index.replenishStockDate}</TableCell>
                          </TableRow>
                        ))}</>
                      ))}
                    </TableBody>
  ...
  return (
    <Layout defaultDrawer={true} setDrawer={setDrawer} title="" tabChildren={<LayoutChild />}>
      <Box display="flex" justifyContent="right" alignItems="center" width="100%" marginBottom="15px">
        <Button
          aria-controls="simple-menu"
          aria-haspopup="true"
          className={classes.resetA}
          onClick={handleItemFiledClick}
        >
          <ArrowDropDownIcon />
          SELECT FIELD
        </Button>
        <Menu
          ...
          anchorEl={itemField}
          keepMounted
          open={Boolean(itemField)}
          onClose={handleCloseFilter}
        >
          {/* ปุ่ม Select Field */}
          {options.map((name) => (
            <MenuItem value={name} onClick={() => handleChangeField(name)} >
              <Checkbox style={{ color: '#8884D8'}} checked={personName.indexOf(name) > -1} value={personName} />
              <ListItemText primary={name} />
            </MenuItem>
          ))}
          <div style={{textAlign: "center"}}>
          <Button aria-controls="simple-menu" className={classes.resetA} onClick={handleClickSubmit}>
            {' '}
            SUMMIT{' '}
          </Button>
          </div>
              {/* <Button aria-controls="simple-menu" className={classes.resetA} onClick={resetFilter1}>
                RESET
              </Button> */}
        </Menu>
        {/* ปุ่ม Select Field */}
